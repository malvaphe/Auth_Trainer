...
function SignIn() {
  ...
  const handleSignin = (e) => {
    e.preventDefault();
    setLoading(true);
    UserService.signin(email, password.current?.value, socket.id, timeNow, null).then(
      () => {
  ...
  const handleSigninWithKey = (myKey) => {
    UserService.signin(email, password.current?.value, socket.id, timeNow, myKey).then(
      (response) => {
  ...
  useEffect(() => {
    socket.on('verifySignIn', (data) => {
      UserService.signin(data.email, password.current?.value, socket.id, timeNow, data.key).then(
        (response) => {
          UserService.getcsrftoken();
          dispatch(set(response.userCredentials));
        },
        ...
        <section className='bg-main-1' data-aos='zoom-y-out'>
          <div className='max-w-6xl mx-auto px-4 sm:px-6'>
            <div className='pt-32 pb-12 md:pt-40 md:pb-20'>
              ...
              <h2 className='h2 bg-clip-text text-white text-center pb-4'>Social authentication</h2>
                ...
                <form onSubmit={handleSignin}>
                  ...
                  {!loading && (
                    <div className='flex flex-wrap -mx-3 mt-6'>
                      <div className='w-full px-3'>
                        <a
                          className='btn px-0 text-white bg-red-600 hover:bg-red-700 w-full relative flex items-center text-center'
